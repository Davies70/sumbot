    ...
    try {
        if (algo === 'neural') {
            if (!summarizerPipeline) {
                self.postMessage({ type: 'status', message: 'Downloading AI Model (~25MB)...' });
                summarizerPipeline = await pipeline('summarization', 'Xenova/distilbart-cnn-6-6', {
                    progress_callback: (data) => {
                        if (data.status === 'progress') self.postMessage({ type: 'progress', value: data.progress });
                    }
                });
            }
            self.postMessage({ type: 'status', message: 'AI is thinking...' });
            const output = await summarizerPipeline(text, { max_new_tokens: 100, iteration_penalty: 1.2 });
            self.postMessage({ type: 'result', summary: output[0].summary_text, isNeural: true });
        } else {
